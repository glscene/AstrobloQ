...
(* -----------------------------------------------------------------------
 START: считывает входной файл и preprocesses the observational data output:
 RSUN:  матрица of three Sun position vectors в эклиптических координатах
 E:     матрица of three observation direction unit vectors
 JD:    юлианская дата of the three observation times
 TEQX:  равноденствие of RSUN and E (в Блианских веках с J2000)
 ----------------------------------------------------------------------- *)
...
begin
  // open input file
  Assign(OrbInput, 'data\ORBINP.DAT');
  Reset(OrbInput);
  ...
  TEQX0 := (EQX0 - 2000.0) / 100.0; // равноденствие, equinox
...
begin
  ...
  writeln('  Склонение            i     ', INC:10:4, ' degrees');
...
begin
  ...
  (* REWRITE(OrbOut); *)
  Assign(OrbOutput, 'data\OrbOut.DAT');
  REWRITE(OrbOutput);
