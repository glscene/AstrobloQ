...
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::FormCreate(TObject *Sender)
{
	miStars->Checked = true;
	miMode->Enabled = false;
	miCatalog->Enabled = false;
	chlbClass->Checked[3] = true;
}


//---------------------------------------------------------------------------
// Èíèöèàëèçàöèÿ ñòðóêòóð Äåëîíå
DelaunayBase __fastcall TFormPÑ::InitDelaunay(String filename, float color[])
{
...
//---------------------------------------------------------------------------
// Init Voronoi struct
VoronoiBase __fastcall TFormPÑ::InitVoronoi(String filename, float color[])
{
	// Connect to specified DB
	FDConnection1->Connected = False;
	FDConnection1->DriverName = "SQLite";
...
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::DrawStars()
{
	// Óäàëåíèå âñåõ çâ¸çä ñöåíû
	GLStars->Free();
	GLStars = (TGLPoints *)(dcGalablock->AddNewChild(__classid(TGLPoints)));

	// Óäàëåíèå âñåõ ëèíèé ñöåíû
	GLLines1->Free();
	...
	for(int i = 0; i < chlbClass->Items->Count; i++)
	{
		if(chlbClass->Checked[i])
		{
			switch (i)
			{
				case 0:
...
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::DrawDelaunay()
{
...
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::DrawVoronoi()
{
	// Óäàëåíèå çâ¸çä ñöåíû
	GLStars->Free();
	GLStars = (TGLPoints *)(dcGalablock->AddNewChild(__classid(TGLPoints)));

	// Óäàëåíèå ëèíèé ñöåíû
	GLLines1->Free();
	GLLines1 = (TGLLines *)(dcGalablock->AddNewChild(__classid(TGLLines)));
...
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::InitDraw()
{
	// Draw 3D model based on selected mode
	if (miStars->Checked == true) {
		DrawStars();
	}
	else if (miTetranet->Checked == true) {
		DrawDelaunay();
	}
	else if (miPolynet->Checked == true) {
		DrawVoronoi();
	}
}
//---------------------------------------------------------------------------
__fastcall TFormPÑ::TFormPÑ(TComponent* Owner)
	: TForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::GLSceneViewer1MouseDown(TObject *Sender, TMouseButton Button,
		  TShiftState Shift, int X, int Y)
{
	mx = X; my = Y;
}
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::GLSceneViewer1MouseMove(TObject *Sender, TShiftState Shift,
		  int X, int Y)
{
...
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::FormMouseWheel(TObject *Sender, TShiftState Shift, int WheelDelta,
...
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::Timer1Timer(TObject *Sender)
{
	FormPÑ->StatusBar1->Panels->Items[0]->Text = \
	Format("×èñëî çâ¸çä: %d", ARRAYOFCONST((GLStars->Positions->Count)));
	FormPÑ->StatusBar1->Panels->Items[1]->Text = \
	Format("FPS: %.2f", ARRAYOFCONST((GLSceneViewer1->FramesPerSecond())));
	GLSceneViewer1->ResetPerformanceMonitor();
}
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::GLCadencer1Progress(TObject *Sender, const double deltaTime,
		  const double newTime)
{
	GLSceneViewer1->Invalidate();
}
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::miExitClick(TObject *Sender)
{
	FormPÑ->Close();
}
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::miStarsClick(TObject *Sender)
{
	if (miStars->Checked == false) miStars->Checked = true;
	FormPÑ->Caption = "HYG 3D | Çâ¸çäû";
	InitDraw();
}
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::miTetranetClick(TObject *Sender)
{
	if (miTetranet->Checked == false) miTetranet->Checked = true;
	FormPÑ->Caption = "HYG 3D | Òåòðàíåò Äåëîíå";
	InitDraw();
}
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::miPolynetClick(TObject *Sender)
{
	if (miPolynet->Checked == false) miPolynet->Checked = true;
	FormPÑ->Caption = "HYG 3D | Ïîëèíåò Âîðîíîãî";
	InitDraw();
}

//---------------------------------------------------------------------------
void __fastcall TFormPÑ::miStartClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::chlbClassClickCheck(TObject *Sender)
{
	// Âûçîâ ìåòîäà draw ïðè êàæäîì âûáîðå checkbox
	InitDraw();
}
//---------------------------------------------------------------------------
void __fastcall TFormPÑ::miCatalogClick(TObject *Sender)
{
	FormDataset->Show();
}
//---------------------------------------------------------------------------
...
*/
void __fastcall TFormPÑ::Starlife1Click(TObject *Sender)
{
   TFormLife*  FormLife;
...
void __fastcall TFormPÑ::miOptionsClick(TObject *Sender)
{
  FormOptions->Show();
}
//---------------------------------------------------------------------------

void __fastcall TFormPÑ::miAboutClick(TObject *Sender)
{
