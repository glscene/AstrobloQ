...
type
  ...
  protected
    ...
    procedure MouseDown(Button_: TMouseButton; Shift_: TShiftState;
      X_, Y_: Single); override;
  ...
  public
    ...
    property Plots[const I_: Integer]: TSingle2D read GetPlot
      write SetPlot; default;
    ...
    property ScaleX[const I_: Integer]: TScaleLine read GetScaleX
      write SetScaleX;
    property ScaleY[const I_: Integer]: TScaleLine read GetScaleY
      write SetScaleY;
...
constructor TScaleLine.Create(const Scale_, Thick_: Single;
...
procedure TScatterPlotFrame.SetScaleX(const I_: Integer;
  const ScaleX_: TScaleLine);
begin
  _ScaleX[I_] := ScaleX_;
  Repaint;
end;
...
procedure TScatterPlotFrame.SetScaleY(const I_: Integer;
  const ScaleY_: TScaleLine);
begin
  _ScaleY[I_] := ScaleY_;
  Repaint;
end;
...
begin
  _FontColor := FontColor_;
  Repaint;
end;

// -----------------------------------------------------------------------

procedure TScatterPlotFrame.MouseDown(Button_: TMouseButton;
...
procedure TScatterPlotFrame.MouseUp(Button_: TMouseButton; Shift_: TShiftState;
  X_, Y_: Single);
begin
  inherited;
...
var
  ...
  P0, P1: TSingle2D;
begin
  X0 := Ceil(_MinX / Interval_);
  X1 := Floor(_MaxX / Interval_);
...
var
  ...
  P0, P1: TSingle2D;
begin
  Y0 := Ceil(_MinY / Interval_);
  Y1 := Floor(_MaxY / Interval_);
...
procedure TScatterPlotFrame.DrawValuesX(const Interval_: Single;
  const Digits_: Integer);
var
  X0, X1, X: Integer;
  P: TSingle2D;
  S: TPointF;
...
begin
  ...
  begin
    ...
    begin
      ...
      with S do
        Y := Y - 0.37 * Font.Size + Font.Size / 2;
...
procedure TScatterPlotFrame.DrawValuesY(const Interval_: Single;
  const Digits_: Integer);
var
  Y0, Y1, Y: Integer;
  P: TSingle2D;
