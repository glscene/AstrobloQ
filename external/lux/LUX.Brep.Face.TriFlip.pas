...
TTriPoin<_TPos_>;
_TFace_:
TTriFace < _TPos_ >> = class(TTreeRoot<_TFace_>)private protected _PoinModel
...
class operator TCornIter<_TPos_>.Equal(const A_, B_: TCornIter<_TPos_>)
  : Boolean;
begin
  Result := (A_.Face = B_.Face) and (A_.Corn = B_.Corn)
end;

class operator TCornIter<_TPos_>.NotEqual(const A_,
  B_: TCornIter<_TPos_>): Boolean;
begin
  Result := not(A_ = B_);
end;
...
class function TTriPoin<_TPos_>.JoinOK(const C1_,
  C2_: TCornIter<_TPos_>): Boolean;
begin
...
var
  T, C, C0, C1, P0, P1, P2, H0, H1, T0, T1, S0, S1, S2, S3: TCornIter<_TPos_>;
begin
...
procedure TTriFace<_TPos_>.SetPoin(const I_: Byte;
  const Poin_: TTriPoin<_TPos_>);
begin
  _Poin[I_] := Poin_;
end;
...
procedure TTriFace<_TPos_>.SetFace(const I_: Byte;
  const Face_: TTriFace<_TPos_>);
begin
  _Face[I_] := Face_;
end;
