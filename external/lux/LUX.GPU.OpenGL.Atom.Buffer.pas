...
type
  ...
  public
    constructor Create(const Paren_: IGLBuffer; const Start_: Pointer;
      const Strid_: GLint);
    ...
    property Items[const I_: Integer]: _TItem_ read GetItems
  ...
  TGLBuffer<_TItem_: record; _TIter_: TGLBufferData<_TItem_>, constructor> =
    class(TGLAtomer, IGLBuffer)
  private
  private type
    _PItem_ = ^_TItem_;
  protected
...
procedure TGLBufferData<_TItem_>.SetItems(const I_: Integer;
  const Item_: _TItem_);
begin
  GetItemsP(I_)^ := Item_;
end;
...
constructor TGLBufferData<_TItem_>.Create(const Paren_: IGLBuffer;
  const Start_: Pointer; const Strid_: GLint);
begin
  inherited Create;
...
procedure TGLBuffer<_TItem_, _TIter_>.SetItems(const I_: Integer;
  const Item_: _TItem_);
...
function TGLBuffer<_TItem_, _TIter_>.Map(const Access_
  : GLenum = GL_READ_WRITE): _TIter_;
begin
  if Assigned(_OnMap) then
    _OnMap(Self);
