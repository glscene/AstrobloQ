...
type
  ...
  TSingleBricIterGridArray3D = class(TBricIterGridArray3D<Single>,
    ISingleBricIterGridArray3D)
  private
  protected
  public
...
procedure TSingleGridArray3D.ForBrics(const Proc_
  : TConstProc<TSingleBricIterGridArray3D>);
begin
  inherited ForBrics(
    procedure(const B_: TBricIterGridArray3D<Single>)
    begin
      Proc_(B_ as TSingleBricIterGridArray3D);
    end);
end;

procedure TSingleGridArray3D.ForEdgesX(const Proc_
  : TConstProc<TSingleBricIterGridArray3D>);
begin
  inherited ForEdgesX(
    procedure(const E_: TBricIterGridArray3D<Single>)
    begin
      Proc_(E_ as TSingleBricIterGridArray3D);
    end);
end;

procedure TSingleGridArray3D.ForEdgesY(const Proc_
  : TConstProc<TSingleBricIterGridArray3D>);
begin
  inherited ForEdgesY(
    procedure(const E_: TBricIterGridArray3D<Single>)
...
procedure TSingleGridArray3D.ForEdgesZ(const Proc_
  : TConstProc<TSingleBricIterGridArray3D>);
begin
  inherited ForEdgesZ(
    procedure(const E_: TBricIterGridArray3D<Single>)
...
function TSingleBricIterGridArray3D.FracGrad(const Xd_, Yd_, Zd_: Single)
  : TSingle3D;
// ······································
  ...
  var
    Y00, Y01, Y10, Y11, Z0, Z1: Single;
  begin
    Y00 := (Grids[X + 1, Y + 0, Z + 0] - Grids[X + 0, Y + 0, Z + 0]) * Xd_ +
      Grids[X + 0, Y + 0, Z + 0];
    Y01 := (Grids[X + 1, Y + 1, Z + 0] - Grids[X + 0, Y + 1, Z + 0]) * Xd_ +
      Grids[X + 0, Y + 1, Z + 0];
    Y10 := (Grids[X + 1, Y + 0, Z + 1] - Grids[X + 0, Y + 0, Z + 1]) * Xd_ +
      Grids[X + 0, Y + 0, Z + 1];
    Y11 := (Grids[X + 1, Y + 1, Z + 1] - Grids[X + 0, Y + 1, Z + 1]) * Xd_ +
      Grids[X + 0, Y + 1, Z + 1];
...
function TSingleBricIterGridArray3D.AbsoGrad(const X_, Y_, Z_: Single)
  : TSingle3D;
var
  Xd, Yd, Zd: Single;
begin
