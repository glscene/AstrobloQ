...
begin
  case I_ of
    0:
      Result := TSingle3D.Create(Min.X, Min.Y, Min.Z);
    1:
      Result := TSingle3D.Create(Max.X, Min.Y, Min.Z);
    2:
      Result := TSingle3D.Create(Min.X, Max.Y, Min.Z);
    3:
      Result := TSingle3D.Create(Max.X, Max.Y, Min.Z);
    4:
      Result := TSingle3D.Create(Min.X, Min.Y, Max.Z);
    5:
      Result := TSingle3D.Create(Max.X, Min.Y, Max.Z);
    6:
      Result := TSingle3D.Create(Min.X, Max.Y, Max.Z);
    7:
      Result := TSingle3D.Create(Max.X, Max.Y, Max.Z);
  end;
end;
...
begin
  Result := TSingleArea2D.Create(Min.X, Min.Y, Max.X, Max.Y);
end;

function TSingleArea3D.GetProjYZ: TSingleArea2D;
begin
  Result := TSingleArea2D.Create(Min.Y, Min.Z, Max.Y, Max.Z);
end;

function TSingleArea3D.GetProjZX: TSingleArea2D;
begin
  Result := TSingleArea2D.Create(Min.Z, Min.X, Max.Z, Max.X);
end;
...
begin
  Result := ProjX.Collision(Area_.ProjX) and ProjY.Collision(Area_.ProjY) and
    ProjZ.Collision(Area_.ProjZ);
end;
...
begin
  case I_ of
    0:
      Result := TDouble3D.Create(Min.X, Min.Y, Min.Z);
    1:
      Result := TDouble3D.Create(Max.X, Min.Y, Min.Z);
    2:
      Result := TDouble3D.Create(Min.X, Max.Y, Min.Z);
    3:
      Result := TDouble3D.Create(Max.X, Max.Y, Min.Z);
    4:
      Result := TDouble3D.Create(Min.X, Min.Y, Max.Z);
    5:
      Result := TDouble3D.Create(Max.X, Min.Y, Max.Z);
    6:
      Result := TDouble3D.Create(Min.X, Max.Y, Max.Z);
    7:
      Result := TDouble3D.Create(Max.X, Max.Y, Max.Z);
  end;
end;
...
begin
  Result := TDoubleArea2D.Create(Min.X, Min.Y, Max.X, Max.Y);
end;

function TDoubleArea3D.GetProjYZ: TDoubleArea2D;
begin
  Result := TDoubleArea2D.Create(Min.Y, Min.Z, Max.Y, Max.Z);
end;

function TDoubleArea3D.GetProjZX: TDoubleArea2D;
begin
  Result := TDoubleArea2D.Create(Min.Z, Min.X, Max.Z, Max.X);
end;
...
begin
  Result := A_.X * B_.X + A_.Y * B_.Y + A_.Z * B_.Z;
end;
