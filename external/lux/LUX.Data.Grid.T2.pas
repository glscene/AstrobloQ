...
type
  ...
  public
    ...
    constructor Create(const ItemsX_, ItemsY_, MargsX_, MargsY_: Integer);
      overload; virtual;
    ...
    property MargsY: Integer read GetMargsY write SetMargsY;
    property ItemsN: Integer read GetItemsN;
    property Items[const X_, Y_: Integer]: _TItem_ read GetItems
  ...
  public
    property Cells[const X_, Y_: Integer]: _TItem_ read GetItems
      write SetItems; default;
    property CellsX: Integer read GetItemsX write SetItemsX;
    property CellsY: Integer read GetItemsY write SetItemsY;
    property PoinsX: Integer read GetPoinsX write SetPoinsX;
  ...
  protected
    ...
    procedure SetCellsY(const CellsY_: Integer);
  public
    constructor Create(const CellsX_, CellsY_, MargsX_,
      MargsY_: Integer); override;
    destructor Destroy; override;
    property Poins[const X_, Y_: Integer]: _TItem_ read GetItems
      write SetItems; default;
    property PoinsX: Integer read GetItemsX write SetItemsX;
    property PoinsY: Integer read GetItemsY write SetItemsY;
    property CellsX: Integer read GetCellsX write SetCellsX;
...
procedure TArray2D<_TItem_>.SetElems(const X_, Y_: Integer;
  const Elem_: _TItem_);
begin
...
procedure TArray2D<_TItem_>.SetItems(const X_, Y_: Integer;
  const Item_: _TItem_);
begin
...
constructor TArray2D<_TItem_>.Create(const ItemsX_, ItemsY_, MargsX_,
  MargsY_: Integer);
begin
...
begin
  ...
  begin
    ...
    for EX := -_MargsX to -1 do
      Items[EX, EY] := Items[EX + _ItemsX, MY];
    for EX := 0 to _ItemsX - 1 do
      Items[EX, EY] := Items[EX, MY];
    for EX := _ItemsX to _ItemsX + _MargsX - 1 do
      Items[EX, EY] := Items[EX - _ItemsX, MY];
  end;
  ...
  begin
    for EX := -_MargsX to -1 do
      Items[EX, EY] := Items[EX + _ItemsX, EY];
...
begin
  ...
  for Y := 00 to HY do
  begin
    for X := 00 - MX to 00 - 01 do
      Items[X, Y] := Items[X + NX, Y];
    for X := HX + 01 to HX + MX do
      Items[X, Y] := Items[X - NX, Y];
  end;

  for X := 00 - MX to HX + MX do
  begin
    for Y := 00 - MY to 00 - 01 do
      Items[X, Y] := Items[X, Y + NY];
    for Y := HY + 01 to HY + MY do
      Items[X, Y] := Items[X, Y - NY];
  end;
end;
...
begin
  ...
  begin
    for X := 00 - MX to 00 - 01 do
      Items[X, Y] := Items[00 - X - 01, Y];
    for X := HX + 01 to HX + MX do
      Items[X, Y] := Items[HX - X + NX, Y];
  end;

  for X := 00 - MX to HX + MX do
  begin
    for Y := 00 - MY to 00 - 01 do
      Items[X, Y] := Items[X, 00 - Y - 01];
    for Y := HY + 01 to HY + MY do
      Items[X, Y] := Items[X, HY - Y + NY];
  end;
end;
...
constructor TPoinArray2D<_TItem_>.Create(const CellsX_, CellsY_, MargsX_,
  MargsY_: Integer);
begin
  inherited Create(CellsX_ + 1, CellsY_ + 1, MargsX_, MargsY_);
...
begin
  ...
  begin
    for X := 00 - MX to 00 - 01 do
      Items[X, Y] := Items[X + HX, Y];
    for X := HX + 00 to HX + MX do
      Items[X, Y] := Items[X - HX, Y];
  end;
  ...
  begin
    for Y := 00 - MY to 00 - 01 do
      Items[X, Y] := Items[X, Y + HY];
    for Y := HY + 00 to HY + MY do
      Items[X, Y] := Items[X, Y - HY];
  end;
end;
