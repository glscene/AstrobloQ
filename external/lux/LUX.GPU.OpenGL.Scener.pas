...
type
  ...
  IGLObject = interface
    ...
    function HitBouBox(const AbsoRay_: TSingleRay3D;
      out Len_: TSingleArea): Boolean;
    procedure HitRay(const AbsoRay_: TSingleRay3D; var Len_: Single;
      var Obj_: TGLObject); overload;
  ...
  public
    ...
    function HitBouBox(const AbsoRay_: TSingleRay3D;
      out Len_: TSingleArea): Boolean;
    procedure HitRay(const AbsoRay_: TSingleRay3D; var Len_: Single;
      var Obj_: TGLObject); overload; virtual;
    function HitRay(const AbsoRay_: TSingleRay3D): TGLObject; overload;
  end;
...
begin
  ...
  RunFamily(
    procedure(const Node_: TTreeNode)
    begin
...
function TGLObject.HitBouBox(const AbsoRay_: TSingleRay3D;
out Len_: TSingleArea): Boolean;
// ······································
...
begin
  ...
  begin
    ...
    begin
      if Vec.X > 0 then
        Slab(Min.X, Max.X, Pos.X, Vec.X)
      else if Vec.X < 0 then
        Slab(Max.X, Min.X, Pos.X, Vec.X)
      else if (Pos.X < Min.X) or (Max.X < Pos.X) then
        Exit;

      if Vec.Y > 0 then
        Slab(Min.Y, Max.Y, Pos.Y, Vec.Y)
      else if Vec.Y < 0 then
        Slab(Max.Y, Min.Y, Pos.Y, Vec.Y)
      else if (Pos.Y < Min.Y) or (Max.Y < Pos.Y) then
        Exit;

      if Vec.Z > 0 then
        Slab(Min.Z, Max.Z, Pos.Z, Vec.Z)
      else if Vec.Z < 0 then
        Slab(Max.Z, Min.Z, Pos.Z, Vec.Z)
      else if (Pos.Z < Min.Z) or (Max.Z < Pos.Z) then
        Exit;
    end;
...
procedure TGLObject.HitRay(const AbsoRay_: TSingleRay3D; var Len_: Single;
var Obj_: TGLObject);
var
  L: TSingleArea;
  I: Integer;
