...
type
  ...
  public
    ...
    class function Inner(const P1_, P2_, P3_, P4_: TSingle3D)
      : TSingleSphere; static;
  ...
  public
    ...
    class function Inner(const P1_, P2_, P3_, P4_: TDouble3D)
      : TDoubleSphere; static;
  ...
  public
    ...
    class operator Multiply(const A_: TSingleTria3D; const B_: Single)
      : TSingleTria3D; inline;
    class operator Multiply(const A_: Single; const B_: TSingleTria3D)
      : TSingleTria3D; inline;
    class operator Divide(const A_: TSingleTria3D; const B_: Single)
      : TSingleTria3D; inline;
  ...
  public
    ...
    class operator Multiply(const A_: TDoubleTria3D; const B_: Double)
      : TDoubleTria3D; inline;
    class operator Multiply(const A_: Double; const B_: TDoubleTria3D)
      : TDoubleTria3D; inline;
    class operator Divide(const A_: TDoubleTria3D; const B_: Double)
      : TDoubleTria3D; inline;
...
function Barycenter(const P0_, P1_, P2_, P3_: TSinglePos3D)
  : TSinglePos3D; overload;
function Barycenter(const P0_, P1_, P2_, P3_: TDoublePos3D)
  : TDoublePos3D; overload;
...
function InnerCenter(const P0_, P1_, P2_, P3_: TSinglePos3D)
  : TSinglePos3D; overload;
function InnerCenter(const P0_, P1_, P2_, P3_: TDoublePos3D)
  : TDoublePos3D; overload;

function CircumCenter(const P1_, P2_, P3_, P4_: TSinglePos3D)
  : TSinglePos3D; overload;
function CircumCenter(const P1_, P2_, P3_, P4_: TDoublePos3D)
  : TDoublePos3D; overload;
...
function MarginCorner(const V1_, V2_: TSingleVec3D; Margin_: Single)
  : TSingleVec3D; overload;
function MarginCorner(const V1_, V2_: TDoubleVec3D; Margin_: Double)
  : TDoubleVec3D; overload;

function MarginCorner(const P0_, P1_, P2_: TSinglePos3D; Margin_: Single)
  : TSinglePos3D; overload;
function MarginCorner(const P0_, P1_, P2_: TDoublePos3D; Margin_: Double)
  : TDoublePos3D; overload;
...
function ReflectW(const Ray_, Nor_: TSingle3D; const RaI_: Single)
  : Single; overload;
function ReflectW(const Ray_, Nor_: TDouble3D; const RaI_: Double)
  : Double; overload;

function Refract(const Ray_, Nor_: TSingle3D; const RaI_: Single)
  : TSingle3D; overload;
function Refract(const Ray_, Nor_: TDouble3D; const RaI_: Double)
  : TDouble3D; overload;

function Refract(const Ray_, Nor_: TSingle3D; const RaI_: Single;
  out RaV_: TSingle3D; out ReW_: Single): Boolean; overload;
function Refract(const Ray_, Nor_: TDouble3D; const RaI_: Double;
  out RaV_: TDouble3D; out ReW_: Double): Boolean; overload;

function SolidAngle(const P1_, P2_, P3_: TSingle3D; const P_: TSingle3D)
  : Single; overload;
function SolidAngle(const P1_, P2_, P3_: TDouble3D; const P_: TDouble3D)
  : Double; overload;
...
class operator TSingleSphere.Implicit(const Spher2_: TSingleSpher2)
  : TSingleSphere;
begin
  with Result do
  begin
...
class operator TSingleSphere.Implicit(const Sphere_: TSingleSphere)
  : TSingleSpher2;
begin
  with Result do
  begin
...
class function TSingleSphere.Inner(const P1_, P2_, P3_, P4_: TSingle3D)
  : TSingleSphere;
begin
  with Result do
  begin
...
class operator TDoubleSphere.Implicit(const Spher2_: TDoubleSpher2)
  : TDoubleSphere;
begin
  with Result do
  begin
...
class operator TDoubleSphere.Implicit(const Sphere_: TDoubleSphere)
  : TDoubleSpher2;
begin
  with Result do
  begin
...
class function TDoubleSphere.Inner(const P1_, P2_, P3_, P4_: TDouble3D)
  : TDoubleSphere;
begin
  with Result do
  begin
...
begin
  with Poin1 do
    Result.Poin1 := TSingle2D.Create(X, Y);
  with Poin2 do
    Result.Poin2 := TSingle2D.Create(X, Y);
  with Poin3 do
    Result.Poin3 := TSingle2D.Create(X, Y);
end;

function TSingleTria3D.GetProjYZ: TSingleTria2D;
begin
  with Poin1 do
    Result.Poin1 := TSingle2D.Create(Y, Z);
  with Poin2 do
    Result.Poin2 := TSingle2D.Create(Y, Z);
  with Poin3 do
    Result.Poin3 := TSingle2D.Create(Y, Z);
end;

function TSingleTria3D.GetProjZX: TSingleTria2D;
begin
  with Poin1 do
    Result.Poin1 := TSingle2D.Create(Z, X);
  with Poin2 do
    Result.Poin2 := TSingle2D.Create(Z, X);
  with Poin3 do
    Result.Poin3 := TSingle2D.Create(Z, X);
end;
...
class operator TSingleTria3D.Subtract(const A_, B_: TSingleTria3D)
  : TSingleTria3D;
begin
  with Result do
  begin
...
class operator TSingleTria3D.Multiply(const A_: TSingleTria3D; const B_: Single)
  : TSingleTria3D;
begin
  with Result do
  begin
...
class operator TSingleTria3D.Multiply(const A_: Single; const B_: TSingleTria3D)
  : TSingleTria3D;
begin
  with Result do
  begin
...
class operator TSingleTria3D.Divide(const A_: TSingleTria3D; const B_: Single)
  : TSingleTria3D;
begin
  with Result do
  begin
...
// ······································
begin
  Result := AABB.Collision(Area_) and CheckPlane and
    ProjXY.ColliEdge(Area_.ProjXY) and ProjYZ.ColliEdge(Area_.ProjYZ) and
    ProjZX.ColliEdge(Area_.ProjZX);
end;
...
begin
  ...
  Result := AABB.Collision(Area_) // Check( AX ) and Check( AY ) and Check( AZ )
    ...
    and Check(CrossProduct(AX, E1)) and Check(CrossProduct(AX, E2)) and
    Check(CrossProduct(AX, E3)) and Check(CrossProduct(AY, E1)) and
    Check(CrossProduct(AY, E2)) and Check(CrossProduct(AY, E3)) and
    Check(CrossProduct(AZ, E1)) and Check(CrossProduct(AZ, E2)) and
    Check(CrossProduct(AZ, E3));
end;
...
begin
  with Poin1 do
    Result.Poin1 := TDouble2D.Create(X, Y);
  with Poin2 do
    Result.Poin2 := TDouble2D.Create(X, Y);
  with Poin3 do
    Result.Poin3 := TDouble2D.Create(X, Y);
end;

function TDoubleTria3D.GetProjYZ: TDoubleTria2D;
begin
  with Poin1 do
    Result.Poin1 := TDouble2D.Create(Y, Z);
  with Poin2 do
    Result.Poin2 := TDouble2D.Create(Y, Z);
  with Poin3 do
    Result.Poin3 := TDouble2D.Create(Y, Z);
end;

function TDoubleTria3D.GetProjZX: TDoubleTria2D;
begin
  with Poin1 do
    Result.Poin1 := TDouble2D.Create(Z, X);
  with Poin2 do
    Result.Poin2 := TDouble2D.Create(Z, X);
  with Poin3 do
    Result.Poin3 := TDouble2D.Create(Z, X);
end;
...
class operator TDoubleTria3D.Subtract(const A_, B_: TDoubleTria3D)
  : TDoubleTria3D;
begin
  with Result do
  begin
...
class operator TDoubleTria3D.Multiply(const A_: TDoubleTria3D; const B_: Double)
  : TDoubleTria3D;
begin
  with Result do
  begin
...
class operator TDoubleTria3D.Multiply(const A_: Double; const B_: TDoubleTria3D)
  : TDoubleTria3D;
begin
  with Result do
  begin
...
class operator TDoubleTria3D.Divide(const A_: TDoubleTria3D; const B_: Double)
  : TDoubleTria3D;
begin
  with Result do
  begin
...
// ······································
var
  NX0, NY0, NZ0, NX1, NY1, NZ1: TSingle3D;
begin
  with Self do
  begin
    NX0 := NorvX;
  ...
  Result := Check(NX0) and Check(NY0) and Check(NZ0) and Check(NX1) and
    Check(NY1) and Check(NZ1) and Check(CrossProduct(NX0, NX1)) and
    Check(CrossProduct(NX0, NY1)) and Check(CrossProduct(NX0, NZ1)) and
    Check(CrossProduct(NY0, NX1)) and Check(CrossProduct(NY0, NY1)) and
    Check(CrossProduct(NY0, NZ1)) and Check(CrossProduct(NZ0, NX1)) and
    Check(CrossProduct(NZ0, NY1)) and Check(CrossProduct(NZ0, NZ1));
end;
...
// ······································
var
  NX0, NY0, NZ0, NX1, NY1, NZ1: TDouble3D;
begin
  with Self do
  begin
    NX0 := NorvX;
...
begin
  ...
  Result := Roo2(Pow2(L1 + L2 + L3) - 2 * (Pow2(L1) + Pow2(L2) + Pow2(L3))) / 4;
end;
...
var
  L01, L23, LL1, L02, L31, LL2, L03, L12, LL3: Single;
begin
  L01 := Distanc2(P0_, P1_);
  L23 := Distanc2(P2_, P3_);
  L02 := Distanc2(P0_, P2_);
  L31 := Distanc2(P3_, P1_);
  L03 := Distanc2(P0_, P3_);
  L12 := Distanc2(P1_, P2_);

  LL1 := L01 + L23;
  LL2 := L02 + L31;
  LL3 := L03 + L12;

  Result := (L23 * (L01 * (LL2 + LL3 - LL1) - L02 * L03) + L31 *
    (L02 * (LL3 + LL1 - LL2) - L03 * L01) + L12 * (L03 * (LL1 + LL2 - LL3) - L01
    * L02) - L23 * L31 * L12) / 144;
end;

function HeronVolum2(const P0_, P1_, P2_, P3_: TDoublePos3D): Double;
var
  L01, L23, LL1, L02, L31, LL2, L03, L12, LL3: Double;
begin
  L01 := Distanc2(P0_, P1_);
  L23 := Distanc2(P2_, P3_);
  L02 := Distanc2(P0_, P2_);
...
begin
  ...
  Result := (A0 * P0_ + A1 * P1_ + A2 * P2_ + A3 * P3_) / (A0 + A1 + A2 + A3);
end;
...
begin
  ...
  Result := (W1 * P1_ + W2 * P2_ + W3 * P3_) / (W1 + W2 + W3);
end;

function CircumSphereCenter(const P1_, P2_, P3_: TDouble3D): TDouble3D;
var
  L1, L2, L3, W1, W2, W3: Double;
begin
  L1 := Distance(P2_, P3_);
  L2 := Distance(P3_, P1_);
  L3 := Distance(P1_, P2_);
...
begin
  ...
  begin
    ...
    Result := (Pow2((C + G) / (C - G)) + Pow2((N2C + G) / (N2C - G))) / 2;
    {
...
function Refract(const Ray_, Nor_: TSingle3D; const RaI_: Single;
  out RaV_: TSingle3D; out ReW_: Single): Boolean;
var
  N2, C, C2, G, G2, N2C: Single;
begin
  N2 := Pow2(RaI_);
  ...
  begin
    ...
    RaV_ := (Ray_ - (C + G) * Nor_) / RaI_;
    RaV_ := RaV_.Unitor;

    N2C := N2 * C;

    ReW_ := (Pow2((C + G) / (C - G)) + Pow2((N2C + G) / (N2C - G))) / 2;
    {
      R := Pow2( ( RaI_ - 1 ) / ( RaI_ + 1 ) );
...
function Refract(const Ray_, Nor_: TDouble3D; const RaI_: Double;
  out RaV_: TDouble3D; out ReW_: Double): Boolean;
var
  N2, C, C2, G, G2, N2C: Double;
begin
  N2 := Pow2(RaI_);
...
function SolidAngle(const P1_, P2_, P3_: TSingle3D;
  const P_: TSingle3D): Single;
var
  V1, V2, V3: TSingle3D;
  L1, L2, L3, A, B: Single;
begin
  ...
  A := V1.X * V2.Y * V3.Z + V2.X * V3.Y * V1.Z + V3.X * V1.Y * V2.Z - V1.X *
    V3.Y * V2.Z - V2.X * V1.Y * V3.Z - V3.X * V2.Y * V1.Z;

  B := L1 * L2 * L3 + L3 * DotProduct(V1, V2) + L1 * DotProduct(V2, V3) + L2 *
    DotProduct(V3, V1);

  Result := 2 * ArcTan2(A, B);
end;

function SolidAngle(const P1_, P2_, P3_: TDouble3D;
  const P_: TDouble3D): Double;
var
  V1, V2, V3: TDouble3D;
  L1, L2, L3, A, B: Double;
begin
