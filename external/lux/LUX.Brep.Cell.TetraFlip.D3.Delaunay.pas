...
type
  ...
  public
    ...
    function AddCell(const Poin0_, Poin1_, Poin2_, Poin3_: TDelaPoin)
      : TDelaCell;
    ...
    procedure AddPoin3(const Poin_: TDelaPoin; const Cell_: TDelaCell);
      overload;
  end;
...
begin
  ...
  begin
    if Open < 0 then
      Result := ((Pos_ - Center).Siz2 < Radiu2)
    else
      Result := (DotProduct(Pos_, Center) < Radiu2);
  end;
end;
...
function TDelaunay3D.AddCell(const Poin0_, Poin1_, Poin2_, Poin3_: TDelaPoin)
  : TDelaCell;
var
  I: Shortint;
begin
...
begin
  ...
  if _PoinModel.ChildsN > 3 then
    AddPoin3(Result)
  else if _PoinModel.ChildsN = 3 then
    Init3;
end;
...
// ----------------------------------------------------------------------------
  function Tree(const Cell_: TDelaCell; const Vert_: Byte;
    const E1_, E2_, E3_: Byte): TJoint;
  // --------------------------------------------------------------------------------------
        ...
        0:
          begin
            ...
            begin
              ...
              with _BondTable[Bond[V1]] do
                J1 := Tree(Cell[V1], Vert[V1], _[E1_], _[E3_], _[E2_]);
              with _BondTable[Bond[V2]] do
                J2 := Tree(Cell[V2], Vert[V2], _[E2_], _[E1_], _[E3_]);
              with _BondTable[Bond[V3]] do
                J3 := Tree(Cell[V3], Vert[V3], _[E3_], _[E2_], _[E1_]);
        ...
        2:
          begin
            ...
            begin
              Cell1 := Cell[V1];
              Vert1 := Vert[V1];
              Edge1 := _BondTable[Bond[V1]]._[E1_];
              Cell2 := Cell[V2];
              Vert2 := Vert[V2];
              Edge2 := _BondTable[Bond[V2]]._[E2_];
              Cell3 := Cell[V3];
              Vert3 := Vert[V3];
              Edge3 := _BondTable[Bond[V3]]._[E3_];
            end;
          end;
      end;
    end;
...
begin
  ...
  begin
    ...
    with _BondTable[Bond[0]] do
      J0 := Tree(Cell[0], Vert[0], _[1], _[2], _[3]);
    with _BondTable[Bond[1]] do
      J1 := Tree(Cell[1], Vert[1], _[1], _[2], _[3]);
    with _BondTable[Bond[2]] do
      J2 := Tree(Cell[2], Vert[2], _[1], _[2], _[3]);
    with _BondTable[Bond[3]] do
      J3 := Tree(Cell[3], Vert[3], _[1], _[2], _[3]);
