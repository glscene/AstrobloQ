...
type
  ...
  private
    procedure InitFace;
    function FaceTree(const Poin_: _TPoin_; const Face_: _TFace_;
      const Vert_: Byte): TFaceJoint;
    procedure Connect(const J_, JL_, JR_: TFaceJoint);
  protected
    function NewPoin(const Pos_: TSingle2D): _TPoin_;
    function NewFace(const Poin1_, Poin2_, Poin3_: _TPoin_;
      const Open_: Byte): _TFace_;
  public
    ...
    function AddPoin3(const Pos_: TSingle2D; const Face_: _TFace_)
      : _TPoin_; overload;
...
const
  VertTable: array [1 .. 3] of TVertLR = ((L: 2; R: 3), (L: 3; R: 1),
    (L: 1; R: 2));

  OpenTable: array [1 .. 3, 1 .. 3, 0 .. 3] of Byte =
    (((0, 0, 3, 2), (0, 2, 0, 3), (0, 3, 2, 0)), ((0, 0, 1, 3), (0, 3, 0, 1),
    (0, 1, 3, 0)), ((0, 0, 2, 1), (0, 1, 0, 2), (0, 2, 1, 0)));
...
begin
  case _Open of
    ...
    1:
      Result := EdgeCircle(Poin[2], Poin[3]);
    2:
      Result := EdgeCircle(Poin[3], Poin[1]);
    3:
      Result := EdgeCircle(Poin[1], Poin[2]);
  end;
end;
...
begin
  ...
  begin
    if Open = 0 then
      Result := (Distance(Pos_, Center) < Radius)
    else
      Result := (DotProduct(Pos_, Center) < Radius);
  end;
end;
...
function TDelaunay2D<_TPoin_, _TFace_>.FaceTree(const Poin_: _TPoin_;
  const Face_: _TFace_; const Vert_: Byte): TFaceJoint;
var
  JL, JR: TFaceJoint;
  C: _TFace_;
begin
  ...
  begin
    with VertTable[Vert_] do
      C := NewFace(Face_.Poin[L], Poin_, Face_.Poin[R],
        OpenTable[2, Vert_, Face_.Open]);
...
function TDelaunay2D<_TPoin_, _TFace_>.NewFace(const Poin1_, Poin2_,
  Poin3_: _TPoin_; const Open_: Byte): _TFace_;
begin
...
function TDelaunay2D<_TPoin_, _TFace_>.HitCircleFace
  (const Pos_: TSingle2D): _TFace_;
var
  I: Integer;
begin
  ...
  begin
    ...
    if Result.IsHitCircle(Pos_) then
      Exit;
  end;
...
function TDelaunay2D<_TPoin_, _TFace_>.AddPoin3(const Pos_: TSingle2D;
  const Face_: _TFace_): _TPoin_;
begin
  Result := NewPoin(Pos_);
  AddPoin3(Result, Face_);
end;
