...
type
  ...
  public
    ...
    procedure DrawCircle(const Center_: TPointF; const Radius_: Single;
      const Opacity_: Single = 1);
    procedure FillCircle(const Center_: TPointF; const Radius_: Single;
      const Opacity_: Single = 1);
    procedure DrawText(const Text_: String; const Pos_: TPointF;
      const AlignX_: TTextAlign; const AlignY_: TTextAlign;
      const Opacity_: Single = 1);
  end;
...
procedure HCanvas.DrawCircle(const Center_: TPointF; const Radius_: Single;
  const Opacity_: Single = 1);
begin
...
procedure HCanvas.FillCircle(const Center_: TPointF; const Radius_: Single;
  const Opacity_: Single = 1);
begin
...
procedure HCanvas.DrawText(const Text_: String; const Pos_: TPointF;
  const AlignX_: TTextAlign; const AlignY_: TTextAlign;
  const Opacity_: Single = 1);
var
  W, H, W2, H2: Single;
...
procedure HMeshData.SaveToFileBinSTL(const FileName_: String;
  const Text_: AnsiString = '');
var
  ...
  begin
    ...
    begin
      if F is TControl3D then
        TControl3D(F).RecalcFamilyAbsolute;
    end;
  end;
end;
...
begin
  ...
  begin
        ...
        begin
          if BeginScene then
            try
              ...
              Lights.Clear;
              for I := 0 to Camera.Viewport.LightCount - 1 do
                Lights.Add(Camera.Viewport.Lights[I].LightDescription);

              for C in _RenderingList do
              begin
                with C do
                begin
                  if Visible or (not Visible and (csDesigning in ComponentState)
                    and not Locked) then
                    RenderInternalTo(Self.Context);
                end;
              end;
    ...
    inherited Canvas.DrawBitmap(_Bitmap, _Bitmap.BoundsF, LocalRect,
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (F is TControl3D) then
          _RenderingList.Add(F as TControl3D);
      end;
...
begin
  ...
  begin
    ...
    DrawLines(_GeometryX.VertexBuffer, _GeometryX.IndexBuffer,
      _MaterialX.Material, AbsoluteOpacity);
    DrawLines(_GeometryY.VertexBuffer, _GeometryY.IndexBuffer,
      _MaterialY.Material, AbsoluteOpacity);
    DrawLines(_GeometryZ.VertexBuffer, _GeometryZ.IndexBuffer,
...
begin
  ...
  begin
    case MinI(Abs(X), Abs(Y), Abs(Z)) of
      1:
        E := TPoint3D.Create(1, 0, 0);
      2:
        E := TPoint3D.Create(0, 1, 0);
      3:
        E := TPoint3D.Create(0, 0, 1);
    end;
