...
begin
  ...
  begin
    ...
    with T.AABB do
    begin
      X0 := Floor(Min.X / _Reso);
      X1 := Floor(Max.X / _Reso);
      Y0 := Floor(Min.Y / _Reso);
      Y1 := Floor(Max.Y / _Reso);
      Z0 := Floor(Min.Z / _Reso);
      Z1 := Floor(Max.Z / _Reso);
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if T.CollisionPEF(B) and not Vs.ContainsKey(V) then
            Vs.Add(V, 0);
        end;
      end;
    end;
  ...
  begin
    DivL := _DivL;
    Area := TSingleArea3D.Create(_Cent.X - _Size / 2, _Cent.Y - _Size / 2,
      _Cent.Z - _Size / 2, _Cent.X + _Size / 2, _Cent.Y + _Size / 2,
      _Cent.Z + _Size / 2);
...
begin
  ...
  _Tree.ForFamily(
    procedure(const Node_: IOcNode3D)
    var
      L: Cardinal;
      I: TCardinal3D;
    begin
  ...
  begin
    ...
    V.Pose := TSingleM4.Translate(_Cent.X - _Size / 2, _Cent.Y - _Size / 2,
      _Cent.Z - _Size / 2);
  end;
end;
