...
type
  ...
  public
    ...
    class procedure TestDriverSupport(out DriverType_: D3D_DRIVER_TYPE;
      out FeatureLevel_: TD3D_FEATURE_LEVEL);
...
type
  ...
  private
    ...
    class procedure FindBestShaderSource(const Shader_: TContextShader;
...
begin
  ...
  for I := 0 to High(Source_.Variables) do
    Result := Result + Max(Source_.Variables[I].Size, 16);
end;

function D3D11CreateDevice1Ex(DriverType_: D3D_DRIVER_TYPE; Flags_: LongWord;
  out Device_: ID3D11Device; out Context_: ID3D11DeviceContext;
...
function TLuxCustomDX11Context.GetIndexBufferSupport
  : TContext3D.TIndexBufferSupport;
begin
  if _FeatureLevel > D3D_FEATURE_LEVEL_9_1 then
...
class procedure TLuxCustomDX11Context.TestDriverSupport(out DriverType_
  : D3D_DRIVER_TYPE; out FeatureLevel_: TD3D_FEATURE_LEVEL);
var
  DX11Lib: THandle;
  Device: ID3D11Device;
    ...
    if DX11Lib <> 0 then
      try
        ...
        begin
          SaveClearFPUState;
          try
            if (not GlobalUseDXSoftware) and
              Succeeded(D3D11CreateDevice1Ex(D3D_DRIVER_TYPE_HARDWARE,
              D3D11_CREATE_DEVICE_BGRA_SUPPORT, Device, Context, _FeatureLevel))
            then
            begin
              Device := nil;
            ...
            else if Succeeded(D3D11CreateDevice1Ex(D3D_DRIVER_TYPE_WARP,
              D3D11_CREATE_DEVICE_BGRA_SUPPORT, Device, Context, _FeatureLevel))
            then
            begin
              Device := nil;
...
begin
  ...
  case _FeatureLevel of
    ...
    begin
      if _FeatureLevel > D3D_FEATURE_LEVEL_11_0 then
        Result := 16384
      else
        Result := 2048;
    end;
...
begin
  ...
  while (Result < UInt(_Resources.Count)) and (_Resources[Result] <> nil) do
    Inc(Result);

  if Result < UInt(_Resources.Count) then
    _Resources[Result] := Resource_
  else
    Result := _Resources.Add(Resource_);
end;

class procedure TLuxDX11Context.RemoveResource(Resource_: THandle);
begin
  if (_Resources <> nil) and (Resource_ <> 0) then
...
begin
  if Multisample_ = TMultisample.FourSamples then
    MultisampleCount := 4
  else if Multisample_ = TMultisample.TwoSamples then
    MultisampleCount := 2
  else
    MultisampleCount := 1;
...
procedure TLuxDX11Context.SetTexture(const Unit_: Integer;
  const Texture_: TTexture);
begin
  if Texture_ is TTexture3D then
    SetTexture3D(Unit_, Texture_ as TTexture3D)
  else
    SetTexture2D(Unit_, Texture_);
end;

procedure TLuxDX11Context.SetTexture2D(const Unit_: Integer;
  const Texture_: TTexture);
var
  Tex: ID3D11Texture2D;
...
begin
  ...
  try
    ...
    begin
      ...
      if Succeeded(HR) then
      begin
        DXGIFactory.MakeWindowAssociation(WindowHandleToPlatform(Parent).Wnd,
...
begin
  ...
  try
    ...
    begin
      ...
      if Failed(SharedDevice.CreateTexture2D(Desc, nil, _CopyBuffer)) then
        Exit;
    end;
...
begin
  ...
  try
    ...
    if _SavedViewportNum > 0 then
      SharedContext.RSSetViewports(1, @_SavedViewport);
...
begin
  ...
  try
    ...
    begin
      ...
      if TClearTarget.Depth in Target_ then
...
begin
  case Fail_ of
    ...
    TStencilOp.Zero:
      _DepthStencilDesc.FrontFace.StencilFailOp := D3D11_STENCIL_OP_ZERO;
    TStencilOp.Replace:
      _DepthStencilDesc.FrontFace.StencilFailOp := D3D11_STENCIL_OP_REPLACE;
    TStencilOp.Increase:
      _DepthStencilDesc.FrontFace.StencilFailOp := D3D11_STENCIL_OP_INCR_SAT;
    TStencilOp.Decrease:
      _DepthStencilDesc.FrontFace.StencilFailOp := D3D11_STENCIL_OP_DECR_SAT;
    TStencilOp.Invert:
      _DepthStencilDesc.FrontFace.StencilFailOp := D3D11_STENCIL_OP_INVERT;
  end;
  ...
  case ZPass_ of
    ...
    TStencilOp.Zero:
      _DepthStencilDesc.FrontFace.StencilPassOp := D3D11_STENCIL_OP_ZERO;
    TStencilOp.Replace:
      _DepthStencilDesc.FrontFace.StencilPassOp := D3D11_STENCIL_OP_REPLACE;
    TStencilOp.Increase:
      _DepthStencilDesc.FrontFace.StencilPassOp := D3D11_STENCIL_OP_INCR_SAT;
    TStencilOp.Decrease:
      _DepthStencilDesc.FrontFace.StencilPassOp := D3D11_STENCIL_OP_DECR_SAT;
    TStencilOp.Invert:
      _DepthStencilDesc.FrontFace.StencilPassOp := D3D11_STENCIL_OP_INVERT;
  end;
...
begin
  case Func_ of
    TStencilfunc.Never:
      _DepthStencilDesc.FrontFace.StencilFunc := D3D11_COMPARISON_NEVER;
    TStencilfunc.Less:
      _DepthStencilDesc.FrontFace.StencilFunc := D3D11_COMPARISON_LESS;
    TStencilfunc.Lequal:
      _DepthStencilDesc.FrontFace.StencilFunc := D3D11_COMPARISON_LESS_EQUAL;
    TStencilfunc.Greater:
      _DepthStencilDesc.FrontFace.StencilFunc := D3D11_COMPARISON_GREATER;
    TStencilfunc.Gequal:
      _DepthStencilDesc.FrontFace.StencilFunc := D3D11_COMPARISON_GREATER_EQUAL;
    TStencilfunc.Equal:
      _DepthStencilDesc.FrontFace.StencilFunc := D3D11_COMPARISON_EQUAL;
    TStencilfunc.NotEqual:
      _DepthStencilDesc.FrontFace.StencilFunc := D3D11_COMPARISON_NOT_EQUAL;
    TStencilfunc.Always:
    ...
    try
      ...
      if Succeeded(SharedContext.Map(_IB, 0, Flags, 0, Mapped)) then
        try
          ...
          begin
            for I := 0 to IndexCount_ - 1 do
              PWord(NativeInt(Mapped.pData) + _IBLockPos + I * SizeOf(Word))^ :=
                PLongInt(NativeInt(Indices_) + I * SizeOf(LongInt))^;
          end
          else
            Move(Indices_^, PLongInt(NativeInt(Mapped.pData) + _IBLockPos)^,
              IndexCount_ * PhysIndexSize);
        finally
      ...
      begin
        ...
        begin
          ...
          if Desc.ByteWidth = UInt(Length(_VSBuf)) then
            NeedCreateVS := False;
        end;
      ...
      begin
        ...
        begin
          ...
          if Desc.ByteWidth = UInt(Length(_PSBuf)) then
            NeedCreatePS := False;
        end;
      ...
      begin
        ...
        if PhysIndexSize = SizeOf(LongInt) then
          ...
          SharedContext.IASetIndexBuffer(_IB, DXGI_FORMAT_R16_UINT, _IBLockPos);
...
begin
  if Texture_ is TTexture3D then
    DoInitializeTexture3D(Texture_ as TTexture3D)
  else
    DoInitializeTexture2D(Texture_);
end;
...
begin
  ...
  if Texture_.PixelFormat = TPixelFormat.None then
    Texture_.PixelFormat := TPixelFormat.BGRA;
  ...
  begin
    if TTextureStyle.Dynamic in Texture_.Style then
      Desc.MipLevels := 1
    else
      Desc.MipLevels := 0;
  end
          ...
          try
            ...
            begin
              ...
              begin
                ...
                begin
                  Move(Lines[Y, Z]^, PByteArray(Mapped.pData)
                    [Mapped.DepthPitch * Z + Mapped.RowPitch * Y], LineSize);
                end;
              end;
            end;
          finally
...
begin
  if Bitmap_.CanvasClass.InheritsFrom(TCustomCanvasGpu) then
    ...
    Result := inherited DoBitmapToTexture(Bitmap_);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Move(Data_[0], _VSBuf[Source.Variables[I].Index],
          Min(SizeOf(Data_), Source.Variables[I].Size));
  ...
  begin
    ...
    begin
      if SameText(Source.Variables[I].Name, Name_) then
      begin
        Move(Data_[0], _PSBuf[Source.Variables[I].Index],
          Min(SizeOf(Data_), Source.Variables[I].Size));
...
begin
  ...
  if DriverType <> D3D_DRIVER_TYPE_NULL then
    TContextManager.RegisterContext(TLuxDX11Context, True);
end;
