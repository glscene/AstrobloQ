...
type
  ...
  public
    ...
    property Pixels[const X_, Y_, Z_: Integer]: _TPixel_ read GetPixels
      write SetPixels;
  end;
...
procedure TTexture3D.ContextLostHandler(const Sender: TObject;
  const Msg: TMessage);
begin
  if not(TTextureStyle.Volatile in Style) then
  begin
    if Handle <> 0 then
      _RequireInitializeAfterLost := True;

    Finalize;
  end;
end;

procedure TTexture3D.ContextResetHandler(const Sender: TObject;
  const Msg: TMessage);
begin
  if not(TTextureStyle.Volatile in Style) then
  begin
    if _RequireInitializeAfterLost then
      Initialize;
...
begin
  ...
  _ContextLostId := TMessageManager.DefaultManager.SubscribeToMessage
...
begin
  if Assigned(_Map) then
    TContextManager.DefaultContextClass.UpdateTexture(Self, _Map.Lines[0, 0],
      _Map.StepY);
end;
...
procedure TTexture3D<_TPixel_>.SetPixels(const X_, Y_, Z_: Integer;
  const Pixel_: _TPixel_);
begin
  Map[X_, Y_, Z_] := Pixel_;
end;
