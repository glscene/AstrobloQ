...
type
  ...
  public
    ...
    constructor Create(const ItemsX_, ItemsY_, ItemsZ_,
      Margs_: Integer); overload;
    constructor Create(const ItemsX_, ItemsY_, ItemsZ_, MargsX_, MargsY_,
      MargsZ_: Integer); overload; virtual;
    procedure AfterConstruction; override;
    destructor Destroy; override;
    property ItemByte: Integer read GetItemByte;
    property ElemsP0: Pointer read GetElemsP0;
    property Elems[const X_, Y_, Z_: Integer]: _TItem_ read GetElems
      write SetElems;
    ...
    property ElemsN: Integer read GetElemsN;
    property ElemsByte: Integer read GetElemsByte;
    property Items[const X_, Y_, Z_: Integer]: _TItem_ read GetItems
  ...
  protected
    ...
    procedure SetGridsZ(const GridZ_: Integer);
  public
    property Brics[const X_, Y_, Z_: Integer]: _TItem_ read GetItems
      write SetItems; default;
    property BricsX: Integer read GetItemsX write SetItemsX;
    property BricsY: Integer read GetItemsY write SetItemsY;
    property BricsZ: Integer read GetItemsZ write SetItemsZ;
  ...
  public
    constructor Create(const BricsX_, BricsY_, BricsZ_, MargsX_, MargsY_,
      MargsZ_: Integer); override;
    destructor Destroy; override;
    property Grids[const X_, Y_, Z_: Integer]: _TItem_ read GetItems
      write SetItems; default;
    property GridsX: Integer read GetItemsX write SetItemsX;
    property GridsY: Integer read GetItemsY write SetItemsY;
    property GridsZ: Integer read GetItemsZ write SetItemsZ;
    ...
    procedure ForBrics(const Proc_: TConstProc < TBricIterGridArray3D <
      _TItem_ >> );
    procedure ForEdgesX(const Proc_: TConstProc < TBricIterGridArray3D <
      _TItem_ >> );
    procedure ForEdgesY(const Proc_: TConstProc < TBricIterGridArray3D <
      _TItem_ >> );
    procedure ForEdgesZ(const Proc_: TConstProc < TBricIterGridArray3D <
      _TItem_ >> );
  end;
  ...
  TBricIterGridArray3D<_TItem_> = class(TInterfacedObject,
    IBricIterGridArray3D<_TItem_>)
  private
  protected type
    _PItem_ = ^_TItem_;
  protected
...
begin
  Result := ((_MargsZ + Z_) * _ElemsY + (_MargsY + Y_)) * _ElemsX +
    (_MargsX + X_);
end;
...
procedure TArray3D<_TItem_>.SetElems(const X_, Y_, Z_: Integer;
  const Elem_: _TItem_);
begin
...
procedure TArray3D<_TItem_>.SetItems(const X_, Y_, Z_: Integer;
  const Item_: _TItem_);
begin
...
constructor TArray3D<_TItem_>.Create(const ItemsX_, ItemsY_, ItemsZ_,
  Margs_: Integer);
begin
  Create(ItemsX_, ItemsY_, ItemsZ_, Margs_, Margs_, Margs_);
end;

constructor TArray3D<_TItem_>.Create(const ItemsX_, ItemsY_, ItemsZ_, MargsX_,
  MargsY_, MargsZ_: Integer);
begin
  inherited Create;
...
constructor TGridArray3D<_TItem_>.Create(const BricsX_, BricsY_, BricsZ_,
  MargsX_, MargsY_, MargsZ_: Integer);
begin
  inherited Create(BricsX_ + 1, BricsY_ + 1, BricsZ_ + 1, MargsX_,
    MargsY_, MargsZ_);
...
procedure TGridArray3D<_TItem_>.ForBrics(const Proc_: TConstProc <
...
function TBricIterGridArray3D<_TItem_>.GetGrids(const X_, Y_,
  Z_: Shortint): _TItem_;
begin
  Result := _Grids[Z_, Y_, X_]^;
end;

procedure TBricIterGridArray3D<_TItem_>.SetGrids(const X_, Y_, Z_: Shortint;
const Item_: _TItem_);
begin
...
function TBricIterGridArray3D<_TItem_>.GetGi(const X_, Y_, Z_: Shortint)
  : TInteger3D;
begin
  with Result do
  begin
...
constructor TBricIterGridArray3D<_TItem_>.Create(const Array_
...
var
  Y, Z: Integer;
begin
  Move(_GX[-1], _GX[0], 3 * SizeOf(Integer));
  Dec(_GX[-1]);
...
var
  Y, Z: Integer;
begin
  Move(_GX[0], _GX[-1], 3 * SizeOf(Integer));
  Inc(_GX[+2]);
...
var
  Z, X: Integer;
begin
  Move(_GY[-1], _GY[0], 3 * SizeOf(Integer));
  Dec(_GY[-1]);

  for Z := -1 to +2 do
  begin
    Move(_HeadY[Z, -1], _HeadY[Z, 0], 3 * SizeOf(_PItem_));
    Dec(_HeadY[Z, -1], _Paren.CounStepY);

    Move(_Grids[Z, -1], _Grids[Z, 0], 12 * SizeOf(_PItem_));

    for X := -1 to +2 do
      Dec(_Grids[Z, -1, X], _Paren.CounStepY);
  end;
end;
...
var
  Z, X: Integer;
begin
  Move(_GY[0], _GY[-1], 3 * SizeOf(Integer));
  Inc(_GY[+2]);

  for Z := -1 to +2 do
  begin
    Move(_HeadY[Z, 0], _HeadY[Z, -1], 3 * SizeOf(_PItem_));
    Inc(_HeadY[Z, +2], _Paren.CounStepY);

    Move(_Grids[Z, 0], _Grids[Z, -1], 12 * SizeOf(_PItem_));

    for X := -1 to +2 do
      Inc(_Grids[Z, +2, X], _Paren.CounStepY);
  end;
end;
...
var
  X, Y: Integer;
begin
  Move(_GZ[-1], _GZ[0], 3 * SizeOf(Integer));
  Dec(_GZ[-1]);

  Move(_HeadZ[-1], _HeadZ[0], 3 * SizeOf(_PItem_));
  Dec(_HeadZ[-1], _Paren.CounStepZ);
  ...
  begin
    for X := -1 to +2 do
      Dec(_Grids[-1, Y, X], _Paren.CounStepZ);
  end;
end;
...
var
  X, Y: Integer;
begin
  Move(_GZ[0], _GZ[-1], 3 * SizeOf(Integer));
  Inc(_GZ[+2]);

  Move(_HeadZ[0], _HeadZ[-1], 3 * SizeOf(_PItem_));
  Inc(_HeadZ[+2], _Paren.CounStepZ);
...
function TBricIterGridArray3D<_TItem_>.Interp(const Xd_, Yd_,
  Zd_: Single): _TItem_;
begin
  Result := Grids[Round(Xd_), Round(Yd_), Round(Zd_)];
end;

function TBricIterGridArray3D<_TItem_>.AbsoInterp(const X_, Y_,
  Z_: Single): _TItem_;
var
  Xd, Yd, Zd: Single;
begin
