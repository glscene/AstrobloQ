...
begin
  ...
  try
    ...
    IniFile.WriteInteger(frmSettings.Name, ceStyles.Name, ActiveLangID);
  finally
    IniFile.Free;
  end;
  inherited;
...
begin
  ...
  IniFile := TIniFile.Create(ChangeFileExt(ParamStr(0), '.ini'));
  try
    ActiveLangId := IniFile.ReadInteger(frmSettings.Name, ceStyles.Name, 0);
...
/// CheckBoxRotate.Checked := IniFile.ReadBool(frmSettings.Name, CheckBoxRotate.Name, True);
    case ActiveLangId of
      LANG_ENGLISH: ceStyles.ItemIndex := 0;
      LANG_RUSSIAN: ceStyles.ItemIndex := 1;
      LANG_PORTUGUESE: ceStyles.ItemIndex := 2;
      LANG_SPANISH: ceStyles.ItemIndex := 3;
    else
      ceStyles.ItemIndex := 0;
    end;
  finally
    IniFile.Free;
  end;
