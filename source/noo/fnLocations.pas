...
var
  ...
  sDateFormat, sName, sNickName, sCity, sState, sCountry, sPhoto, sEMail, sUrl,
    sDemoName, sTypeName, sDescription, sDateGLS, sDateDOB,
    sWhoWhereFormat: string;
  mp: TMarkerPosition;
  { iIndex : integer; }
begin
  ...
  try
    ...
    begin
      ...
      *)
      ...
      sWhoWhereFormat := LongitudeEdit.Text + ' ' + LatitudeEdit.Text + ' ' +
        inttostr(byteType) + ' ' + inttostr(byteType2) + ' ' + sTypeName + ',' +
        sName + ',' + sNickName + ',' + sCity + ',' + sState + ',' + sCountry +
        ',' + sDateGLS + ',' + sDateDOB + ',' + sPhoto + ',' + sEMail + ',' +
        sUrl + ',' + sDemoName + ',' + sDescription;
...
procedure TFormLocations.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  //
end;
