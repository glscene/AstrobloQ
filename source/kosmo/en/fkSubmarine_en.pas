...
begin
  ...
  // clockwise rotation
  if IsKeyDown(',') or IsKeyDown('á') then
  begin
    ffSubmarine.Roll(-0.1);
  end;
  // counterclockwise rotation
  if IsKeyDown('.') or IsKeyDown('þ') then
  begin
    ffSubmarine.Roll(0.1);
  end;
  //  moving forward
  ...
  // backward movement
  if IsKeyDown('z') or IsKeyDown('ÿ') then
    if dspeed > -0.5 then
    // speed increment after pressing a key
      dspeed := dspeed - 0.0001;
  (*
...
begin
  case Key of
    ...
    'c','C','ñ','Ñ':  // inside or outside the submarine
      begin
