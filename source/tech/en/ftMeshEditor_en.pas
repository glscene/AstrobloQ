...
procedure TfrmMeshEditor.FormShow(Sender: TObject);
begin
  Application.OnHint := ShowHint;
end;

procedure TfrmMeshEditor.FormCreate(Sender: TObject);
begin
  top := HoloFormY;
  left := HoloFormX;
  (*
...
function TfrmMeshEditor.GetPolygonMode: TGLPolygonMode;
begin
  Result := GLFreeForm1.Material.PolygonMode;
end;

procedure TfrmMeshEditor.SetPolygonMode(const Value: TGLPolygonMode);
begin
  GLFreeForm1.Material.PolygonMode := Value;
end;

procedure TfrmMeshEditor.cbPolygonModeChange(Sender: TObject);
begin
...
procedure TfrmMeshEditor.ShowHint(Sender: TObject);
begin
  // HintPanel.Caption := GetLongHint(Application.Hint);
  StatusBar.Panels[0].Text := Application.Hint;
end;

procedure TfrmMeshEditor.Exit1Click(Sender: TObject);
begin
  Close;
end;

procedure TfrmMeshEditor.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  HoloFormY := frmMeshEditor.top;
  HoloFormX := frmMeshEditor.left;
end;

procedure TfrmMeshEditor.FormDestroy(Sender: TObject);
begin
  FModifierList.Clear;
  FreeAndNil(FModifierList);
end;

procedure TfrmMeshEditor.ScnMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
...
procedure TfrmMeshEditor.ScnMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
var
  { vec1,
    vec2,
...
procedure TfrmMeshEditor.TrackBar1Change(Sender: TObject);
begin
...
procedure TfrmMeshEditor.Open1Click(Sender: TObject);
var
  F: TextFile;
  S: string;
begin
...
procedure TfrmMeshEditor.Save1Click(Sender: TObject);
var
  F: TextFile;
  { I: integer;
    FirstLine: string; }
...
procedure TfrmMeshEditor.ViewControlPanelClick(Sender: TObject);
begin
...
procedure TfrmMeshEditor.Contents1Click(Sender: TObject);
begin
  Application.HelpCommand(HELP_CONTENTS, 0);
end;

procedure TfrmMeshEditor.OnHelp1Click(Sender: TObject);
begin
  Application.HelpCommand(HELP_HELPONHELP, 0);
end;

procedure TfrmMeshEditor.FormMouseWheel(Sender: TObject; Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint; var Handled: Boolean);
begin
...
procedure TfrmMeshEditor.ScnMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  Pick := nil;
  // SelectedObject := nil;
...
procedure TfrmMeshEditor.SetVertexModifiers;
...
procedure TfrmMeshEditor.ShowModifierStatus(const aObj: TModifierCube);
begin
  if aObj = nil then
...
procedure TfrmMeshEditor.ChangeMeshVector(const aObj: TModifierCube;
  const aPos: TVector4f);
var
...
procedure TfrmMeshEditor.chbViewPointsClick(Sender: TObject);
var
  i: Integer;
begin
  GLScene1.BeginUpdate;
...
procedure TfrmMeshEditor.chbShowAxisClick(Sender: TObject);
begin
  dcModifiers.ShowAxes := TCheckBox(Sender).Checked;
end;

procedure TfrmMeshEditor.tbPosChange(Sender: TObject);
begin
  GLCamera1.Position.Z := tbPos.Position;
end;

procedure TfrmMeshEditor.ScnBeforeRender(Sender: TObject);
begin
  glEnable(GL_NORMALIZE);
end;

procedure TfrmMeshEditor.btnVertexClick(Sender: TObject);
var
  i, j: Integer;
  lList: TStringList;
  lVector: TVector3f;
...
procedure TfrmMeshEditor.btnNormalsClick(Sender: TObject);
var
  i, j: Integer;
  lList: TStringList;
  lVector: TVector3f;
...
procedure TfrmMeshEditor.btnTextcoordsClick(Sender: TObject);
var
  i, j: Integer;
  lList: TStringList;
  lVector: TVector3f;
...
procedure TfrmMeshEditor.btnGroupsClick(Sender: TObject);
var
  i: Integer;
  lList: TStringList;
begin
