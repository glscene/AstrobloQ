...
//----------------------------------------------------------------------------
procedure TfrmLoadModel.FormShow(Sender: TObject);
begin
  frmLoadModel.Cursor := crDefault;
end;

//----------------------------------------------------------------------------
procedure TfrmLoadModel.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  FormLoadModelY := frmLoadModel.top;
  FormLoadModelX := frmLoadModel.left;
  ModalResult := mrOK;
end;

//----------------------------------------------------------------------------
procedure TfrmLoadModel.MDLoadBtnClick(Sender: TObject);
begin
...
//----------------------------------------------------------------------------
procedure TfrmLoadModel.DoMdlOpen(const FileName: TFileName);
var
  MDL: file;
  I: integer;
...
begin
  ...
  begin
    Add('ID : ' + Header.ID);
    Add('Версия : ' + IntToStr(Header.Version));
    Add('Имя : ' + Header.Name);
    Add('Length : ' + IntToStr(Header.FileSize));
    Add(Format('EyePosition : %3.2f, %3.2f, %3.2f', [Header.EyePosition[0],
      Header.EyePosition[1], Header.EyePosition[2]]));
    Add(Format('Min : %3.2f, %3.2f, %3.2f', [Header.Min[0], Header.Min[1],
      Header.Min[2]]));
    Add(Format('Max : %3.2f, %3.2f, %3.2f', [Header.Max[0], Header.Max[1],
      Header.Max[2]]));
    Add(Format('BBMin : %3.2f, %3.2f, %3.2f', [Header.BBMin[0], Header.BBMin[1],
      Header.BBMin[2]]));
    Add(Format('BBMax : %3.2f, %3.2f, %3.2f', [Header.BBMax[0], Header.BBMax[1],
      Header.BBMax[2]]));
  ...
  for I := 0 to Header.NumBoneContollers - 1 do
  begin
    BlockRead(MDL, BoneController, SizeOf(BoneController));
    with BoneController do
    begin
  ...
  for I := 0 to Header.NumBones - 1 do
  begin
    ...
    begin
      ...
      LBBones.Items.Add(Format('BoneController : %3d, %3d, %3d, %3d, %3d',
        [BoneController[0], BoneController[1], BoneController[2],
        BoneController[3], BoneController[4], BoneController[5]]));
      LBBones.Items.Add(Format('Value : %3.2f, %3.2f, %3.2f, %3.2f, %3.2f',
        [Value[0], Value[1], Value[2], Value[3], Value[4], Value[5]]));
      LBBones.Items.Add(Format('Scale : %3.2f, %3.2f, %3.2f, %3.2f, %3.2f',
...
//----------------------------------------------------------------------------
procedure TfrmLoadModel.PrintBtnClick(Sender: TObject);
(* FileNameLabel
  LBMDL
  LBBoneController
  LBBones
...
//----------------------------------------------------------------------------
procedure TfrmLoadModel.QcPrintBtnClick(Sender: TObject);
begin
  // The parameter string shows in the print queue under "Document   // name".
  RichEdit1.Print(QcFileNameLabel.Caption);
end;

procedure TfrmLoadModel.QcLoadBtnClick(Sender: TObject);
begin
...
procedure TfrmLoadModel.QcSaveBtnClick(Sender: TObject);
begin
