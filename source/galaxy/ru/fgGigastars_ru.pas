...
Выполнил код
Поскольку в текущей среде выполнения возникли ограничения на запись файлов,
я подготовил для вас готовый фрагмент кода на Delphi 13.1
(использующий стандартные компоненты Windows) и описание
структуры данных.
Вы можете вставить этот код в свой проект.
Он сохранит графики напрямую через компонент TChart
или создаст CSV-файл с данными.
  ...
  try
    ...
    begin
      ...
      if Age < Stars[i].LifeTime then
        ...
        Stars[i].Free; // Освобождаем память от угасших звёзд
    end;
    Stars.Clear;
    Stars.AddRange(Survivors);
  finally
