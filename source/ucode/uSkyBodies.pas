...
type
  ...
  TGLStarData = record
    StarName: array [0 .. 9] of Char; // материал в GLMatLibStars
    distance: Single;
    diameter: Single;
    AngleAxis: Single;
    Mass: Single;
...
procedure NormCrossProd(const v1,v2:array of single; var Result:array of single);
// Переход от декартовых к полярным координатам
procedure CartToPol(v: array of single; var Theta, Phi: single);

// Создание куба радиуса 1 (размером size = 2 с вершинами в [-1, 1])
procedure BuildUnitCube(const Tile: double = 1);
// Построение сферы типа Polyhedron и разбиения Subdivide из OpenGL Red Book
procedure BuildSphere(var Mesh: TGLMeshObject; const Depth: integer = 0);
// Звёзды, случайно рассеянные в сфере радиуса 1
procedure BuildStarField(const nbStars: integer);

(*
  Фрактал для построения астероида potatoid случайной формы и размера.
  Параметр Depth соответствует глубине рекурсивного процесса.
  Используйте ActualDepth для генерации такого же potatoid при
...
begin
// RandSeed := Seed;
// MasterAsteroidF:=TGLFreeForm(dumMasters.AddNewChild(TFreeForm));
...
begin
  ...
  try
    for i := 1 to 11 do
    begin // рандомизация ребер Edges
      for j := 0 to i - 1 do
      begin
        New(Edges[i, j]);
...
//
// Алгоритмы создания сферы в виде Polyhedron и разбиения Subdivide из OpenGL Red Book
// procedure BuildSphere(var Mesh:TGLMeshObject; const Depth:integer=0);
//
// {sub}procedure Subdivide(const v1,v2,v3:array of single;const Depth:integer);
// var
// v12,v23,v31	:array[0..2] of single;
// i			:integer;
// begin
