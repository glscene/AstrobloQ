...
var
  FormSpacePilot: TFormSpacePilot;
...
procedure TFormSpacePilot.FormCreate(Sender: TObject);
begin
  ...
  timeMultiplier := 1;
  deltaTimeGlobal := 0;
  if FileExists(ExtractFilePath(ParamStr(0)) + 'EarthScene.chm') THEN
    Application.HelpFile := ExtractFilePath(ParamStr(0)) + 'EarthScene.chm';
end;

procedure TFormSpacePilot.FormShow(Sender: TObject);
begin
  Timer1.Enabled := True;
  GLCadencer1.Enabled := True;
end;

procedure TFormSpacePilot.FormHide(Sender: TObject);
begin
  CBPlay.Checked := False;
  Timer1.Enabled := False;
...
procedure TFormSpacePilot.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
...
procedure TFormSpacePilot.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  { SkyPilotFormY := SkyPilotForm.top;
    SkyPilotFormX := SkyPilotForm.left; }
end;

procedure TFormSpacePilot.FormResize(Sender: TObject);
begin
  GLSceneViewer1.ResetPerformanceMonitor;
end;

procedure TFormSpacePilot.LoadBtnClick(Sender: TObject);
    ...
    try
      ...
      begin
        ...
        begin
          ...
          showmessage(EarthProjectPath + 'Sun.jpg is missing');
...
procedure TFormSpacePilot.SpeedButton1Click(Sender: TObject);
begin
  Application.HelpContext(5000);
end;

procedure TFormSpacePilot.GLCadencer1Progress(Sender: TObject; const deltaTime, newTime: Double);
begin
...
procedure TFormSpacePilot.GLSceneViewer1MouseDown(Sender: TObject; Button: TMouseButton;
...
procedure TFormSpacePilot.GLSceneViewer1MouseEnter(Sender: TObject);
begin
  GLSceneViewer1.SetFocus; // GLSceneViewer.Focused;
end;

procedure TFormSpacePilot.GLSceneViewer1MouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFormSpacePilot.FormKeyPress(Sender: TObject; var Key: Char);
begin
  case Key of
    #27:
      Close;
...
procedure TFormSpacePilot.FormMouseWheel(Sender: TObject; Shift: TShiftState; WheelDelta: Integer;
  MousePos: TPoint; var Handled: Boolean);
var
  f: Single;
begin
...
procedure TFormSpacePilot.Timer1Timer(Sender: TObject);
begin
  Caption := 'Space Pilot: ' + Format('%.1f FPS', [GLSceneViewer1.FramesPerSecond]);
  GLSceneViewer1.ResetPerformanceMonitor;
end;

procedure TFormSpacePilot.TrackBarChange(Sender: TObject);
begin
  MishMash;
end;

{
  ...
  Comet Halley }
procedure TFormSpacePilot.MishMash;
var
  t: Double;
Begin
  { if CBPlay.Checked and Visible then
...
procedure TFormSpacePilot.CBPlayClick(Sender: TObject);
begin
  Timer1.Enabled := True;
  GLCadencer1.Enabled := True;
end;
