...
procedure TfrmCyborg.FormCreate(Sender: TObject);
begin
  top := FormCyborgY;
  left := FormCyborgX;
...
procedure TfrmCyborg.FormShow(Sender: TObject);
begin
  Timer1.Enabled := False;
  GLCadencer1.Enabled := False;
  frmCyborg.Cursor := crDefault;
end;

procedure TfrmCyborg.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Timer1.Enabled := False;
  GLCadencer1.Enabled := False;
  // NoGLRunning:=True;
...
procedure TfrmCyborg.ExitBtnClick(Sender: TObject);
begin
  Close; // ModalResult := mrOK;
end;

procedure TfrmCyborg.HelpBtnClick(Sender: TObject);
begin
  Application.HelpContext(8000);
end;

procedure TfrmCyborg.CheckBox1Click(Sender: TObject);
begin
  Actor1.OverlaySkeleton := CheckBox1.Checked;
end;

procedure TfrmCyborg.GLSceneViewer1MouseDown(Sender: TObject;
...
procedure TfrmCyborg.GLSceneViewer1MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  if Shift <> [] then
  begin
...
procedure TfrmCyborg.Timer1Timer(Sender: TObject);
begin
  Caption := Format('%.1f FPS', [GLSceneViewer1.FramesPerSecond]);
  GLSceneViewer1.ResetPerformanceMonitor;
end;

procedure TfrmCyborg.GLCadencer1Progress(Sender: TObject;
  const deltaTime, newTime: Double);
begin
...
procedure TfrmCyborg.Button1Click(Sender: TObject);
begin
...
procedure TfrmCyborg.DoCcOpen(const fileName: String);
var
  t: Integer;
begin
  // TigerPath := ExtractFilePath(FileName);
  Application.ProcessMessages;
  frmCyborg.Cursor := crHourGlass;
  Timer1.Enabled := True;
...
procedure TfrmCyborg.cboAnimationsChange(Sender: TObject);
begin
...
procedure TfrmCyborg.TrackBar1Change(Sender: TObject);
begin
  Cube1.Position.SetPoint(TrackBar1.Position, TrackBar2.Position,
    TrackBar3.Position);
end;

procedure TfrmCyborg.TrackBar4Change(Sender: TObject);
begin
  Cube2.Position.SetPoint(TrackBar4.Position, TrackBar5.Position,
    TrackBar6.Position);
end;

procedure TfrmCyborg.Panel2MouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
var
  Tb1, Tb2: TTrackBar;
begin
