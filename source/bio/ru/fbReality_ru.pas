...
var
  FormAbout: TfrmAbout;
begin
  FormAbout := TfrmAbout.Create(self);
  FormAbout.ShowModal;
  FormAbout.Free;
end;
...
begin
  ...
  myFormOptions.UserSettings := FormFirst.UserSettings;
...
begin
  ...
  if cbCollisions.Checked then
    FormFirst.Construction.AddEvent('Включить столкновения')
  else
    FormFirst.Construction.AddEvent('Отключить столкновения');
end;
...
begin
  ...
  if cbAI.Checked then
    FormFirst.Construction.AddEvent('Показать биосферу')
  else
    FormFirst.Construction.AddEvent('Скрыть биосферу');
end;
