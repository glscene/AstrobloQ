...
begin
  if Target.ValidTarget then
  begin
    if (myCamera=nil) and FormFirst.UserSettings.Auto3DView
...
begin
  ...
  begin
    ...
    if (MessageDlg('Организм мёрт. Удалить окно?',
...
///   frmFirstForm.RealityForm.ManagerForm.SpaceForm.GLScene.CurrentCamera.Remove(myCamera, false);
end;
...
begin
  ...
  begin
    StopViewer;
    cbView.Enabled := false;
    ShowMessage('Для этого организма нет вида в 3D');
    exit;
  end;
