...
type
  TFormBioneta = class(TForm)
    ...
    PlanetModel: TGLFreeForm;      // Планета типа TGLFreeForm !
...
begin
  ...
  FormFirst.Construction.AddEvent('Çàãðóçêà ôàéëîâ');
  LoadMaterialLibrary;
  LoadModels;
  GenerateTextureMap;
  ...
  begin
    FormFirst.Construction.AddEvent('Àêòèâàöèÿ Bass');
    GLBass.Active := true;
    if not GLBass.Active then
      FormFirst.Construction.AddEventFailure(' Íåò ïîääåðæêè çâóêà!');
  end;
  LoadSounds;
  FormFirst.Construction.AddEvent('Ãåíåðàöèÿ çâóêîâîé ñèñòåìû');
...
begin
  ...
  FormFirst.Construction.AddEvent('Çàãðóçêà òåêñòóðû ' + aTexName +
    ' èç ôàéëà: ' + aFile + '...');
  ...
  begin
    ...
    // done.
    FormFirst.Construction.AddEventSuccess(' ñäåëàíî.');
  end
  else
  begin
    ...
    // not found.
    FormFirst.Construction.AddEventFailure(' íå íàéäåíî!');
  end;
end;
...
begin
  FormFirst.Construction.AddUnderlinedEvent('Çàãðóçêà áèáëèîòåêè ìàòåðèàëîâ:');
...
begin
  // Loading model aFile...
  FormFirst.Construction.AddEvent('Çàãðóçêà ìîäåëè èç ôàéëà: ' + aFile
    + '...');
  if FileExists(aFile) then
  begin
...
begin
  FormFirst.Construction.AddUnderlinedEvent('Çàãðóçêà áèáëèîòåêè ìîäåëåé:');
...
// ----------------------------------------------------------------------------
// Load random stars or from a catalog
// ----------------------------------------------------------------------------
...
begin
  // create grey stars
  FormFirst.Construction.AddEvent('Äîáàâëåíèå çâ¸çä');
  for X := 0 to 6 do
...
begin
  ...
  FormFirst.Construction.AddEvent('Äîáàâëåíà ëóíà');
...
begin
  ...
  begin
    ShowMessage('Ìàêñèìóì ÷åòûðå ñîëíöà!');
    exit;
  end;
  ...
  FormFirst.Construction.AddEvent('Äîáàâëåíî ñîëíöå');
...
begin
  ...
  // î÷èñòèòü âñ¸
  FormFirst.Construction.AddEvent('Êëèíèíã ãàëàêòèêè');
  CleanGalaxy;
  ...
  // ñîçäàòü ïëàíåòó
  FormFirst.Construction.AddEvent('Ñîçäàíèå ïëàíåòû');
  BuildFromMap;

  // ñîçäàòü íîâûå îáúåêòû
  FormFirst.Construction.AddEvent('Äîáàâëåíèå îáúåêòîâ');
  CheckCradle;
end;
...
begin
  ...
  FormFirst.Construction.AddEvent('Planet.Map' + ': ' + 'Âûñîòà' + ' = ' +
     IntToStr(Height) + 'Øèðèíà' + ' = ' + IntToStr(Width));
  ...
  FormFirst.Construction.AddEvent('Planet.Mesh: ' + 'Òðåóãîëüíèêîâ' + ' = ' +
    IntToStr(PlanetMesh.TriangleCount) + 'Âåðøèí' + ' = ' +
  ...
  FormFirst.Construction.AddEvent('Planet.Freeform: Òðåóãîëüíèêîâ = ' +
...
begin
  ...
  begin
    mySun := TaiSun(Environment.Things.NewThing(cSun));
    ReportUserEvent('Äîáàâèòü ñîëíöå: ' + mySun.OneLineDisplay);
...
begin
  ...
  ReportUserEvent('Äîáàâèòü ëóíó: ' + myMoon.OneLineDisplay);
  FormFirst.RealityForm.ManagerForm.ListsForm.EditSatellite
    (TaiSatellite(myMoon));
end;
...
begin
  ...
  FormFirst.Construction.AddEvent('Water.Mesh: Òðåóãîëüíèêîâ = ' +
    IntToStr(WaterMesh.TriangleCount) + ' Âåðøèí = ' +
  ...
  FormFirst.Construction.AddEvent('Water.Freeform: Òðåóãîëüíèêîâ = ' +
...
begin
  ...
  begin
    ...
    *)
    ...
    ShowMessage('Ñîõðàíåíèå ôîòî (' + myFormPhotograph.edPhotograph.Text + ')');
  end;
...
begin
  ...
  FormFirst.Construction.AddEvent('Atmosphere.Freeform: Òðåóãîëüíèêîâ = ' +
...
begin
  FormFirst.Construction.AddUnderlinedEvent('Çàãðóçêà çâóêîâ');
...
begin
  FormFirst.Construction.AddEvent('Çàãðóçêà çâóêîâ èç ôàéëà ' + aFileName
    + '... ');
  if FileExists(aFileName) then
  begin
    GLSoundLibrary.Samples.Add.LoadFromFile(aFileName);
    FormFirst.Construction.AddEventSuccess(' âûïîëíåíî.');
  end
  else
    FormFirst.Construction.AddEventFailure(' íå íàéäåíî!');
end;
...
begin
  ...
  begin
    ...
    ReportUserEvent('Äîáàâèòü îñòûâøåå Ñîëíöå: ' + mySun.OneLineDisplay);
...
begin
  ...
  begin
    ...
    then
    begin
      LastAction('Óáèò: ' + ThingNamePlural(myKind));
...
begin
  if not aMode then
  begin
    FormFirst.Construction.AddEvent('Îòêëþ÷èòü òåíè');
    GLShadowVolume.Mode := svmOff;
  end
  else
  begin
    FormFirst.Construction.AddEvent('Âêëþ÷èòü òåíè');
