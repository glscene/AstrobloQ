  ...
  begin
    ...
    begin
      MessageDlg('The current graph data'#13#10 +
        'can not be plotted as y = f(Ш).'#13#10 + 'The range is too large.',
        mtError, [mbOK], 0);
      SwitchButton.Tag := 0;
      PlotAsFx := True;
      Exit;
    ...
    begin { plot as y = f(x) }
      ...
      try
        Between1.Enabled := (CheckListBox.Count > 1) and
          (pos('Ш', CheckListBox.Items[0]) = 0) and
          (pos('Ш', CheckListBox.Items[1]) = 0);
      except
      end;
