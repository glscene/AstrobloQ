...
procedure TFormSMDqc.FormCreate(Sender: TObject);
begin
  top := FormSmdQcY;
  left := FormSmdQcX;
...
procedure TFormSMDqc.FormShow(Sender: TObject);
begin
  Timer1.Enabled := False;
  GLCadencer1.Enabled := False;
  FormSMDqc.Cursor := crDefault;
end;

procedure TFormSMDqc.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Timer1.Enabled := False;
  GLCadencer1.Enabled := False;
  // NoGLRunning:=True;
  FormSmdQcY := FormSMDqc.top;
  FormSmdQcX := FormSMDqc.left;

end;

procedure TFormSMDqc.FormHide(Sender: TObject);
begin
  Timer1.Enabled := False;
  GLCadencer1.Enabled := False;
end;
...
procedure TFormSMDqc.CheckBox1Click(Sender: TObject);
begin
  Actor1.OverlaySkeleton := CheckBox1.Checked;
end;

procedure TFormSMDqc.GLSceneViewer1MouseDown(Sender: TObject;
...
procedure TFormSMDqc.GLSceneViewer1MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  if Shift <> [] then
  begin
...
procedure TFormSMDqc.Timer1Timer(Sender: TObject);
begin
  Caption := Format('%.1f FPS', [GLSceneViewer1.FramesPerSecond]);
  GLSceneViewer1.ResetPerformanceMonitor;
end;

procedure TFormSMDqc.GLCadencer1Progress(Sender: TObject;
  const deltaTime, newTime: Double);
begin
...
procedure TFormSMDqc.DoCcOpen(const fileName: String);
var
  t: Integer;
begin
...
procedure TFormSMDqc.TrackBar1Change(Sender: TObject);
begin
  Cube1.Position.SetPoint(TrackBar1.Position, TrackBar2.Position,
    TrackBar3.Position);
end;

procedure TFormSMDqc.TrackBar4Change(Sender: TObject);
begin
  Cube2.Position.SetPoint(TrackBar4.Position, TrackBar5.Position,
    TrackBar6.Position);
end;

procedure TFormSMDqc.Panel2MouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
var
  Tb1, Tb2: TTrackBar;
begin
