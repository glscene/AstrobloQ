...
// Порт на Delphi оригинального кода с Fortran выполненное FRANCOU & SIMON
// VSOP исходные файлы:
...
const
  ...
  NUM_COEFS = 978; // fixed ?
  NUM_FILES = 6; // один файл для каждого 1500y периода
  NUM_PLANETS = 9;
  //
  // NUM_PERIODS=17122;

var
  // оригинальные имена файлов. Каждый из файлов объёмом по 400 MB !
...
type
  // TVector3D_D - Double вектор типа запись (в RTL TVector3D имеет тип Single)
  // и not long enough to hold вычисления координат планет
  TVector3D_D = record
    x, y, z: Double;
...
type
  // двоичный формат записи в файл, для более быстрого доступа и меньшего объёма, чем текст
...
begin
  ...
  begin
    ...
    try
      ...
      raise Exception.Create('Ошибка открытия файла VSOP2013');
    end;
    ...
    try
      ...
      if (idf <> 2013) or // sanity test in header
        ...
        (ncoef <> 978) then
        Raise Exception.Create('ошибка в файле');
...
begin
  ...
  begin
    ...
    Result := 'файл:' + aFN + crlf + 'per:' + Format('%5.1f', [t1]) + ' - ' + Format('%5.1f', [t2])
...
// пытаемся вычислить позицию и скорость заданной планеты
...
begin
  ...
  begin
    ...
    // вычисление положения и скорости планеты
