...
type
  ...
  public
    constructor Create(const Parent_: TParent_; const HeadI_: Integer = 0;
      const StepX_: Integer = 1);
...
class operator TdSingle.Multiply(const A_: Single; const B_: TdSingle)
  : TdSingle;
begin
  with Result do
  begin
...
class operator TdSingle.Multiply(const A_: TdSingle; const B_: Single)
  : TdSingle;
begin
  with Result do
  begin
...
class operator TdDouble.Multiply(const A_: Double; const B_: TdDouble)
  : TdDouble;
begin
  with Result do
  begin
...
class operator TdDouble.Multiply(const A_: TdDouble; const B_: Double)
  : TdDouble;
begin
  with Result do
  begin
...
begin
  Result := TSingleArea.Create(+Single.MaxValue, -Single.MaxValue);
end;
...
begin
  Result := TDoubleArea.Create(+Double.MaxValue, -Double.MaxValue);
end;
...
constructor TIter1D<TValue_>.Create(const HeadI_: Integer = 0;
  const StepX_: Integer = 1);
begin
  inherited Create;
...
constructor TIter1D<TValue_, TParent_>.Create(const Parent_: TParent_;
  const HeadI_: Integer = 0; const StepX_: Integer = 1);
begin
...
constructor TArrayIter<TValue_, TElement_>.Create(const Parent_
  : TArray<TElement_>; const HeadI_: Integer = 0; const StepX_: Integer = 1);
begin
  inherited;
