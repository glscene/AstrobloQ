...
procedure TFormInfos.FormCreate(Sender: TObject);
begin
...
procedure TFormInfos.FormShow(Sender: TObject);
var
  ExtStr: String;
  i: Integer;
  dc: HDC;
...
procedure TFormInfos.FormResize(Sender: TObject);
begin
  HUDLogo.Position.X := Viewer.Width - 150;
  HUDLogo.Position.Y := Viewer.Height - 100;
  MaxLines := round(((Viewer.Height - 30) - (Viewer.Height / 3)) / 12);
...
procedure TFormInfos.HandleKeys(const deltaTime: Double);
begin
...
procedure TFormInfos.CadencerProgress(Sender: TObject; const deltaTime, newTime: Double);
begin
...
procedure TFormInfos.GLSLShader1Apply(Shader: TGLCustomGLSLShader);
begin
  with Shader do
  begin
...
procedure TFormInfos.GLSpaceText1Progress(Sender: TObject; const deltaTime, newTime: Double);
begin
  GLSpaceText1.TurnAngle := 15 * newTime;
end;

procedure TFormInfos.HUDExtensionsRender(Sender: TObject; var rci: TGLRenderContextInfo);
var
  i, X, ys, Y: Integer;
  Canvas: TGLCanvas;
  r: TRect;
...
procedure TFormInfos.Timer1Timer(Sender: TObject);
var
  ManagerName: String;
begin
  // some stats
