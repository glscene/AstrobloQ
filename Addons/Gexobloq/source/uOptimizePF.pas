...
type
  uint = Word; //typedef unsigned int uint;
  lint = Longint; //typedef long int lint;
  llint = Longint; //typedef long long int llint;
  ullint = Longint; //typedef unsigned long long int ullint;
...
node = record //struct node;

end;

arc = record  //typedef struct arc
	from: ^node;  //struct node *from;
	too: ^node;  //struct node *to;
	flow: uint;    //uint flow;
	capacity: uint;  //uint capacity;
	direction: uint;  //uint direction;
end;

node = record  //typedef struct node
	visited: uint;
	numAdjacent: uint;
	number: uint;
	labell: uint;
	excess: integer;
	parent: ^node;  //struct node *parent;
...
#endif
#ifdef DISPLAY_CUT
procedure displayCut(const gap: Word);
var 
 i: Cardinal;
begin
...
procedure initializeNode (Node *nd, const uint n);
begin
    nd.label := 0;
	nd.excess := 0;
	nd.parent := NULL;
...
//static void
procedure addToStrongBucket (Node *newRoot, Root *rootBucket) 
begin
	if (rootBucket->start)
	{
...
procedure initializeArc (Arc *ac);
{
	ac->from = NULL;
	ac->to = NULL;
	ac->capacity = 0;
...
//static void
procedure SimpleInitialization; //simpleInitialization (void) 
var
	i, size: Integer;
	Arc *tempArc;
...
begin
	...
	for i:=0 to size-1 (i=0; i<size; ++i) 
	{
	...
	for i:= 1 to size do // (i=0; i<size; ++i)
	begin
...
//static void
procedure quickSort (Arc **arr, const uint first, const uint last);
var
...
//static void
procedure minisort (Node *current) ;
var
...
//static void
procedure decompose (Node *excessNode, const uint source, uint *iteration); 
begin
...
//static void
procedure recoverFlow (const uint gap);
begin
...
function main(int argc, char ** argv): Integer; 
begin
