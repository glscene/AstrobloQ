...
begin
  ...
  TableVertex.TableName := ChangeModelTable(DirTinTriangle, DirTinVertex, ModelName);
  case DrawMode of
...
begin
  ...
  begin
    ...
    try
      ...
      ShowContours := TGBGrid(Source).ShowContours;
...
begin
  ...
  try
    ...
    begin
      ...
      K := 0; // getting contour levels in increasing order
      for I := ActiveAttribute.Levels.LevelCount - 1 downto 0 do
      begin
        Za[K] := ActiveAttribute.Levels[I].Value;
        //        ContourLines.LineList.Items[K] := ActiveAttribute.Levels[I].Value;
        Inc(K);
      end;
  ...
  try
    ...
    case ActiveAttribute.AttributeType of
      ...
      begin
        ...
        StippleTable.TableName := ExpandPath(DirMaterial) + 'DEFAULT';
        StippleTable.Open;
...
var
  ...
  LinkTable, NodeTable, ElementTable, MatrixTable: TFileName;
  QueryLink, QueryMatrix, QueryElement, QueryNode: TQuery;
  TableVertex: TTable;

begin
  ElementTable := ModelName;
  NodeTable    := ChangeModelTable(DirMesh2DElement, DirMesh2DNode, ElementTable);
  MatrixTable  := ChangeModelTable(DirMesh2DElement, DirMesh2DMatrix, ElementTable);
...
var
  OldCursor: TCursor;
  VertexName, FacetName, BodyName, LinkName: TFileName;
begin
  OldCursor := Screen.Cursor;
  try
    Screen.Cursor := crHourGlass;
    ...
    BodyName   := ModelName;
    FacetName  := ChangeModelTable(DirSolidBody, DirSolidFacet, BodyName);
    VertexName := ChangeModelTable(DirSolidBody, DirSolidVertex, BodyName);
    LinkName   := ChangeModelTable(DirSolidBody, DirSolids, BodyName);

    Query.SQL.Add('SELECT DISTINCT L.' + fldID_FACET + ', L.' +
      fldID_NO + ', N.X X, N.Y Y, N.Z Z,' + CRLF + '   N."' +
      ActiveAttribute.AttributeName + '" N."G_Attribute"' + CRLF +
      'FROM "' + LinkName + '" L,"' + VertexName + '" N' + CRLF +
      'WHERE L.' + fldID_VERTEX + ' = N.ID' + CRLF + 'ORDER BY L.' +
      fldID_FACET + ', L.' + fldID_NO);
...
function TDrawings.SelectOptions: boolean;
begin

end;

class function TDrawings.ModelType: integer;
begin
  Result := mtDrawings;
end;
